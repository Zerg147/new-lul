...
export function HistoryItem({ item, onDelete }: HistoryItemProps) {
  ...
  const createAbbreviation = (selection: string) => {
    const words = selection.split(' '); // Map each word to its first letter and join them to form the abbreviation
  ...
  return (
    ...
    >
      <a href={`/chat/${item.urlId}`} className="flex w-full relative truncate block">
        {item.description}
        <span className="ml-2 text-sm px-2.5 py-0.5 rounded-full relative text-bolt-elements-item-contentAccent bg-bolt-elements-item-backgroundAccent">
          {item.platform ? createAbbreviation(item.platform) : 'BX'}
        </span>
